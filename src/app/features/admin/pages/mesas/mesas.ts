...
export class Mesas implements OnInit {
  ...
  protected openCreate(): void {
    this.mesaForm.reset({ numero: 1, estado: 'Libre', activo: true });
  ...
  protected openEdit(mesa: MesaDto): void {
    this.mesaForm.reset({ numero: mesa.numero, estado: mesa.estado, activo: mesa.activo });
    this.formError.set(null);
  ...
  protected save(): void {
    ...
    if (this.modalMode() === 'create') {
      this.tableService.createMesa({ numero: v.numero }).subscribe({
        next: mesa => {
          this.mesas.update(ms => [...ms, mesa]);
          this.closeModal();
        },
      ...
      this.tableService.updateMesa(id, { numero: v.numero, estado: v.estado, activo: v.activo }).subscribe({
        next: updated => {
  ...
  protected deleteMesa(mesa: MesaDto): void {
    ...
    this.tableService.deleteMesa(mesa.id).subscribe(() =>
      this.mesas.update(ms => ms.map(m => m.id === mesa.id ? { ...m, activo: false } : m))
    );
  }

  protected restoreMesa(mesa: MesaDto): void {
    this.tableService.updateMesa(mesa.id, { numero: mesa.numero, estado: mesa.estado, activo: true }).subscribe(updated =>
      this.mesas.update(ms => ms.map(m => m.id === mesa.id ? updated : m))
    );
  }
